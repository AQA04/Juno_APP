export function createModal({ title, content, onConfirm, confirmText = 'Guardar' }) {
    ...
    modal.innerHTML = `
        ...
        <div class="modal-actions" style="display: flex; gap: var(--spacing-md); margin-top: var(--spacing-lg); justify-content: center;">
            <button class="btn-cancel" style="flex: 1; background: rgba(255,255,255,0.08); border: none; padding: 12px; border-radius: 12px; color: white; cursor: pointer; display: ${onConfirm ? 'block' : 'none'};">Cancelar</button>
            <button class="btn-confirm" style="${onConfirm ? 'flex: 1;' : 'width: 130px;'} background: linear-gradient(135deg, #b194d9, #7c5cb7); border: none; padding: 12px; border-radius: 12px; color: white; font-weight: 900; cursor: pointer;">${confirmText}</button>
        </div>
    `;
